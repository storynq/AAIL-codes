...
from AAIL_models import  ModifiedEsmForTokenClassification_V2
...
def compute_metrics(p, save_sample_metrics=False, output_csv="sample_metrics.csv", identifiers = None):
    ...
    for i in range(antibody_pred.shape[0]):
        ...
        if len(valid_labels) > 0:
            ...
            sample_metrics['ab_pred_pos_rate'] = np.mean(binary_preds)  # 预测正样本率
        else:
        ...
        if len(valid_labels) > 0:
            ...
            sample_metrics['ag_pred_pos_rate'] = np.mean(binary_preds)  # 预测正样本率
        else:
