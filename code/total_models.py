...
class ModifiedEsmForAntibodyScreen_V0(EsmPreTrainedModel): 
    def __init__(self, config, config2, word_embedding, word_embedding2, num_labels, ab_step, ag_step):
        super().__init__(config)
        self.num_labels = num_labels
        self.ab_step = ab_step
        ...
        self.feature_extracion = ab_representation_feature_extraction(feature_dim=768, total_step=ab_step)
...
class ModifiedEsmForSequenceClassification_Biomap(EsmPreTrainedModel):
    def __init__(self, config, config2, word_embedding, word_embedding2, num_labels, ab_step, ag_step):
        ...
        self.Ag_ab = AntigenAntibodyModel_Biomap(feature_dim=1024, num_heads=4)
...
class Atten_classficationHead(nn.Module):
    def __init__(self, word_embedding):
        ...
        self.word_linear = nn.Sequential(
            nn.Linear(768,512),
            nn.ReLU(),
            nn.Dropout(0.25))  # for screening used 0.25dropout; for biomap used 0.1dropout
...
class AntigenAntibodyModel_Biomap(nn.Module):
    def __init__(self, feature_dim, num_heads):
        super().__init__()
        self.attention = CrossAttention(num_heads)
        self.fc = nn.Sequential(
